...
class APIClient:
    ...
    def build_url(self, selected_filters: Dict[str, Any] = {}, parameters: Dict[str, Any] = {}) -> str:
        ...
        for filter_name, option_value in selected_filters.items():
            filter_key = self.filter_manager.get_filter_key(self.category_name, filter_name)
            if not filter_key:
                print(f"Filter key for '{filter_name}' not found. Skipping this filter.")
                continue

            if self.filter_manager.is_range_filter(self.category_name, filter_name):
            ...
            elif self.filter_manager.is_interval_filter(self.category_name, filter_name):
                # Handle interval filters
                option_id = self.filter_manager.get_filter_option_id(self.category_name, filter_name, option_value)
                if option_id:
